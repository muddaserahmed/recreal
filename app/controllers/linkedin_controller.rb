...
class LinkedinController < ApplicationController
 before_filter :authenticate_user!
  ...
  @@linkedin_api_key = "cheq81slu4pe"
  @@linkedin_secret_key = "mbCnZofMsah7ilQc"
  #@candidate = current_user.profileable

  def index
    unless LinkedinOauthSetting.find_by_candidate_id(current_user.profileable.id).nil?
  ...
  def oauth_account
    client = LinkedIn::Client.new(@@linkedin_api_key, @@linkedin_secret_key, @@config)
    pin = params[:oauth_verifier]
    if pin
      atoken, asecret = client.authorize_from_request(session[:rtoken], session[:rsecret], pin)
      LinkedinOauthSetting.create!(:asecret => asecret, :atoken => atoken, :candidate_id => current_user.profileable.id)
    end
    redirect_to "/"
  end

  def generate_linkedin_oauth_url
    if LinkedinOauthSetting.find_by_candidate_id(current_user.profileable.id).nil?
      client = LinkedIn::Client.new(@@linkedin_api_key, @@linkedin_secret_key, @@config)
  ...
  def get_client
    linkedin_oauth_setting = LinkedinOauthSetting.find_by_candidate_id(current_user.profileable.id)
    client = LinkedIn::Client.new(@@linkedin_api_key, @@linkedin_secret_key, @@config)
  ...
  def get_basic_profile
    bprofile = BasicProfile.find_by_candidate_id(current_user.profileable.id)
    if bprofile.nil?
      client = get_client
      ...
      new_basic_profile.candidate = current_user.profileable
  ...
  def get_full_profile
    fprofile = FullProfile.find_by_candidate_id(current_user.profileable.id)
    if fprofile.nil?
      client = get_client
      ...
      new_full_profile.candidate = current_user.profileable
  ...
  def get_positions
    positions = Position.find_all_by_full_profile_id(current_user.profileable.full_profile.id)
    if positions.empty?
      client = get_client
      ...
      positions.each do |p|
        if p.is_current == "true"
          Position.create(
            ...
            full_profile_id: current_user.profileable.full_profile.id)
        else
          Position.create(
            title: p.title, 
            summary: p.summary, 
      ...
      current_user.profileable.full_profile.positions
    else
      positions
    end
  end
  ...
  def get_educations
    educations = Education.find_all_by_full_profile_id(current_user.profileable.full_profile.id)
    if educations.empty?
      client = get_client
      ...
      current_user.profileable.full_profile.educations
    else
      educations
    end
  end
